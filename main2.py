...
def main():
    ...
    commands = ['add-contact', 'show-contacts', 'edit-contact', 'delete-contact',
                'upcoming-birthdays', 'add-note', 'show-notes', 'search-notes', 'search-contact', 'exit']       # my 'search-notes'
    ...
    while command != 'exit':
        ...
        elif command == 'search-notes':                     # my all elif
...
def print_menu_commmands():
    ...
    - search-notes        - search for notes in the address book                    
...
def fun_add_contact(address_book, name):
    ...
    phone = input(
        f'Enter the phone of contact {name} (10 digits)(c - close): ')
...
def fun_edit_contact(address_book, contact_name=""):
    ...
    if contact_name in address_book.data:
        ...
        while True:
            edit = input(
                'Enter what you want to edit(p - phone, b - birthday, a - address, e - email) (c - close): ')
            if edit.lower() == 'c':
                break
            try:
                if edit == 'p':
                    ...
                    print(
                        'Ivailid comand, please enter(phone, birthday, address, email) (c - close): ')
            except ValueError:
                edit = input(
                    'Ivailid comand, please enter(phone, birthday, address, email) (c - close): ')
    else:
...
def fun_search_notes(address_book, filename):            # my all fun_search_notes
    # Пошук нотатки за словами або автором                                              
    search_term = input('Enter search term (leave blank to show all notes): ').lower()
    ...
    filtered_notes = [note for note in address_book.notes_manager.notes if search_term in note.text.lower() or search_term in note.author.lower()]
...
class NoteManager:
    ...
    def print_notes(self):
        if self.notes:
            ...
            table.add_column("Index", style="cyan", width=5, justify="center")
            table.add_column("Note", style="green")
            # Додано новий стовпець для автора
            table.add_column("Author", style="blue")
